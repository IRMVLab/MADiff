# Defines HOI transformer backbones for feature encoding and prediction.
import torch
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = x.reshape(B * T, N, x.shape[-1])
        x = x + self.pos_embed
        x = self.pos_drop(x)

        x = x.reshape(B, T, N, x.shape[-1]).permute(0, 2, 1, 3).reshape(B * N, T, x.shape[-1])
        x = self.time_embed(x)
        x = self.time_drop(x)
        x = x.reshape(B, N, T, x.shape[-1]).reshape(B, N * T, x.shape[-1])

        mask = mask.transpose(1, 2)
        for blk in self.encoder_blocks:
            x = blk(x, B, T, N, mask=mask)

        x = x.reshape(B, N, T, x.shape[-1]).permute(0, 2, 1, 3)
        x = self.norm(x)
        return x
...
class ObjectTransformer(nn.Module):
    ...
    def inference(self, feat, bbox_feat, valid_mask, future_valid=None, pred_len=4):
        ...
        return future_hands, pred_contact, feat

    def inference_feat(self, feat, bbox_feat, valid_mask):
        feat = self.encoder_input(feat, bbox_feat)

        return feat

    def inference_feat_and_emb(self, feat, bbox_feat, valid_mask):
        feat = self.encoder_input(feat, bbox_feat)

        return feat

class MyObjectTransformer(nn.Module):
    ...
    def forward(self, feat, bbox_feat, valid_mask):
        feat = self.encoder_input(feat, bbox_feat)
        return feat
