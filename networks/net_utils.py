# Provides tensor masks, initialization, stochastic depth, and trajectory distance helpers.
import torch
...
def get_subsequent_mask(seq, diagonal=1):
    ...
    seq_len = seq.size(1)
    mask = (1 - torch.triu(
        torch.ones((1, seq_len, seq_len), device=seq.device), diagonal=diagonal
    )).bool()
    return mask
...
def traj_affordance_dist(hand_traj, contact_point, future_valid=None, invalid_value=9):
    ...
    selected_dist = dist.min(dim=-1).values
