# Defines attention, feedforward, and transformer blocks.
import torch
...
class MultiHeadAttention(nn.Module):
    ...
    def _reshape_heads(self, x, batch_size, seq_len, channels):
        return x.reshape(batch_size, seq_len, self.num_heads, channels // self.num_heads).transpose(1, 2)

    def forward(self, q, k, v, mask=None):
        B, Nq, Nk, Nv, C = q.shape[0], q.shape[1], k.shape[1], v.shape[1], q.shape[2]
        if self.with_qkv:
            q = self._reshape_heads(self.proj_q(q), B, Nq, C)
            k = self._reshape_heads(self.proj_k(k), B, Nk, C)
            v = self._reshape_heads(self.proj_v(v), B, Nv, C)
        else:
            q = self._reshape_heads(q, B, Nq, C)
            k = self._reshape_heads(k, B, Nk, C)
            v = self._reshape_heads(v, B, Nv, C)
        if mask is not None:
